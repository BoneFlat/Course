...
namespace JackieSoft
{
    public class CellCreator : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            for (var i = 0; i < cellPrefabs.Length; i++)
            {
                var layOutElement = cellPrefabs[i].GetComponent<LayoutElement>();
                var view = cellPrefabs[i].GetComponent<Cell.IView>().GetType();
                _cellPools.Add(view, new Queue<Cell.IView>());
                _cells.Add(view, (cellPrefabs[i], new Vector2(layOutElement.minWidth, layOutElement.minHeight)));
            }
